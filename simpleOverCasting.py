...
#version 150 compatibility
	...
	def Run(self):
		...
		pygame.display.set_caption("OVer CATting")
...
#version 120 compatibility
		...
		''')
		"""
		self.overcaster = compile_program('''
#version 120 compatibility
		// Vertex program
		varying vec3 pos;
		varying vec2 texture_coordinate;
...
#version 120 compatibility
		...
		''')
                glUseProgram(self.overcaster);
                glUniform4(glGetUniformLocation(self.overcaster, "m_Time"), (float)1.0, (float)1.0, (float)1.0, (float)1.0)
		# uniform4: instruction
		# vertices: arguments
		# texture coords: values
		# so it's a function
                for (int pp = 0; pp < 3; ++pp)
                {
                    GL.UseProgram(programs[pp]);
                    
                    GL.BindBuffer(BufferTarget.TextureBuffer, texBufBufQuats);
                    GL.BufferData(BufferTarget.TextureBuffer, (IntPtr)(quats.Length * BlittableValueType.StrideOf(quats)), quats, BufferUsageHint.StaticDraw);
                    GL.BindBuffer(BufferTarget.TextureBuffer, texBufBufPoses);
                    GL.BufferData(BufferTarget.TextureBuffer, (IntPtr)(poses.Length * BlittableValueType.StrideOf(poses)), poses, BufferUsageHint.StaticDraw);
                    GL.BindBuffer(BufferTarget.TextureBuffer, texBufBufOffset);
                    GL.BufferData(BufferTarget.TextureBuffer, (IntPtr)(offsets.Length * BlittableValueType.StrideOf(offsets)), offsets, BufferUsageHint.StaticDraw);
                    GL.BindBuffer(BufferTarget.TextureBuffer, texBufBufScale);
                    GL.BufferData(BufferTarget.TextureBuffer, (IntPtr)(scales.Length * BlittableValueType.StrideOf(scales)), scales, BufferUsageHint.StaticDraw);

                    GL.ActiveTexture(TextureUnit.Texture1);
                    GL.BindTexture(TextureTarget.TextureBuffer, texBufQuats);
                    GL.TexBuffer(TextureBufferTarget.TextureBuffer, SizedInternalFormat.Rgba32f, texBufBufQuats);
                    GL.Uniform1(GL.GetUniformLocation(programs[pp], "quatsTex"), 1);
                    GL.ActiveTexture(TextureUnit.Texture2);
                    GL.BindTexture(TextureTarget.TextureBuffer, texBufPoses);
                    GL.TexBuffer(TextureBufferTarget.TextureBuffer, SizedInternalFormat.Rgba32f, texBufBufPoses);
                    GL.Uniform1(GL.GetUniformLocation(programs[pp], "posesTex"), 2);
                    GL.ActiveTexture(TextureUnit.Texture3);
                    GL.BindTexture(TextureTarget.TextureBuffer, texBufOffset);
                    GL.TexBuffer(TextureBufferTarget.TextureBuffer, SizedInternalFormat.Rgba32f, texBufBufOffset);
                    GL.Uniform1(GL.GetUniformLocation(programs[pp], "offsetTex"), 3);
                    GL.ActiveTexture(TextureUnit.Texture4);
                    GL.BindTexture(TextureTarget.TextureBuffer, texBufScale);
                    GL.TexBuffer(TextureBufferTarget.TextureBuffer, SizedInternalFormat.Rgba32f, texBufBufScale);
                    GL.Uniform1(GL.GetUniformLocation(programs[pp], "scaleTex"), 4);

                    GL.Uniform1(GL.GetUniformLocation(programs[pp], "numJoints"), animation.numJoints);
                    GL.Uniform1(GL.GetUniformLocation(programs[pp], "numInstance"), numInstance);
                    if (rotAngle == 180.0f)
                    {
                        GL.Uniform1(GL.GetUniformLocation(programs[pp], "rotAngleType"), 1);
                    }
                    else if (rotAngle == 270.0f)
                    {
                        GL.Uniform1(GL.GetUniformLocation(programs[pp], "rotAngleType"), 2);
                    }
                    else
                    {
                        GL.Uniform1(GL.GetUniformLocation(programs[pp], "rotAngleType"), 0);
                    }
